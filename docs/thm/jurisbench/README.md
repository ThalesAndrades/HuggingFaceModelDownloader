...
**Benchmark de recuperação semântica jurídica em português brasileiro** — mantido pela [THM Tecnologia](../README.md).
...
Para evitar overfitting e contaminação, **parte do conjunto de avaliação é privada**. Quer ver seu modelo no leaderboard? [Submeta pelo formulário](../../../issues/new?template=02-jurisbench-submissao.yml) — avaliamos e publicamos o resultado.

## Reproduzir a parte pública

```bash
pip install sentence-transformers rank_bm25 datasets
python jurisbench_v0.py
```

Modelos e datasets via hfdownloader (este repositório):
